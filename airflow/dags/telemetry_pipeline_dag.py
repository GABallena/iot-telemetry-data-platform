...
def _timed(step_name, fn, **kwargs):
    ...
    try:
        ...
        record_run(step_name, status="success", rows=rows, duration_seconds=time.time() - t0)
        return result
    except Exception as exc:
        record_run(step_name, status="failure", duration_seconds=time.time() - t0, error=str(exc))
        raise
